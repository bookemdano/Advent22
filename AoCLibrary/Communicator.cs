...
namespace AoCLibrary
{
	...
	{
		static string _cookieString = "53616c7465645f5f4a5c8551a649899923835062e76be6e6079b68e6574d76ad1caeaae367dc68d8d51241b20417f9ce26396c795150a6e4d91eeeedf3d89a03";
        ...
        static public async Task<CommunicatorResult> Read(string url, bool overrideThrottle = false)
        {
            Directory.CreateDirectory(Dir);
            var filename = Path.Combine(Dir, Filename());
            if (overrideThrottle == false &&  File.Exists(filename))
                return new CommunicatorResult(File.ReadAllText(filename), false);
            var uri = new Uri(url);
            ...
            using (var handler  = new HttpClientHandler() { CookieContainer = cookieContainer }) 
            {
                using var client = new HttpClient(handler) { BaseAddress = uri };
                {
                    ...
                    try
                    {
						...
						return new CommunicatorResult(rv, true);
                    }
                    catch (Exception ex)
                    {
						return new CommunicatorResult(ex.ToString(), true);
                    }
                }
            }
	...
	{
		public CommunicatorResult(string json, bool realRead)
		{
			Json = json;
			RealRead = realRead;
		}
